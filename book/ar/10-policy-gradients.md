...
يُعالج **تحسين السياسة القريب** (Proximal Policy Optimization) المشكلتين، وقد
أصبح خطَّ أساس واسع الاستخدام في الروبوتات والألعاب وبعض أنظمة ما بعد تدريب
نماذج اللغة. لكنه ليس مرادفًا لـ RLHF؛ فقد تستخدم المواءمة بالتفضيلات PPO أو
محسّنات سياسة أخرى أو أهدافًا تتعلّم مباشرةً من أزواج التفضيل. فكرته المركزية هي
**الهدف البديل المقصوص** (clipped surrogate objective): تتبّع النسبة بين
السياسة الجديدة والسياسة التي جمعت البيانات، `r = π_new / π_old`، ثم أوقف
الحافز الإضافي خارج المجال `[1−ε, 1+ε]`:
...
هذا يُثبّط التغيّر الكبير في السياسة لكنه لا يضمن استحالته رياضيًّا. وعمليًّا يتيح
لـ PPO أن **يُعيد استخدام كلّ دُفعة لعدّة حِقَب**
